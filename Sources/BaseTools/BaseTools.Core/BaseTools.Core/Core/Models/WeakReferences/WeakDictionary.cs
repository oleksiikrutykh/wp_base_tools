namespace BaseTools.Core.Models.WeakReferences
{
    using System;
    ...
    // Adds strong typing to ExtendedWeakReference.Target using generics. Also,
    ...
    internal class ExtendedWeakReference<T> : WeakReference where T : class 
    {
    public static ExtendedWeakReference<T> Create(T target) {
        if (target == null)
            return WeakNullReference<T>.Singleton;

        return new ExtendedWeakReference<T>(target);
    }

    protected ExtendedWeakReference(T target)
        : base(target, false) { }
...
// legal.
internal class WeakNullReference<T> : ExtendedWeakReference<T> where T : class {
...
internal sealed class WeakKeyReference<T> : ExtendedWeakReference<T> where T : class {
...
/// unreachable. However, left-over ExtendedWeakReference objects for the key
    ...
    where TValue : class
{
    ...
    private Dictionary<object, ExtendedWeakReference<TValue>> dictionary;
    ...
    public WeakDictionary(int capacity, IEqualityComparer<TKey> comparer)
    {
        ...
        this.dictionary = new Dictionary<object, ExtendedWeakReference<TValue>>(capacity, this.comparer);
    }
    ...
    public override void Add(TKey key, TValue value)
    {
        ...
        ExtendedWeakReference<TKey> weakKey = new WeakKeyReference<TKey>(key, this.comparer);
        ExtendedWeakReference<TValue> weakValue = ExtendedWeakReference<TValue>.Create(value);
    ...
    {
        ExtendedWeakReference<TValue> weakValue;
    ...
    {
        ExtendedWeakReference<TKey> weakKey = new WeakKeyReference<TKey>(key, this.comparer);
        this.dictionary[weakKey] = ExtendedWeakReference<TValue>.Create(value);
    }
    ...
    {
        foreach (KeyValuePair<object, ExtendedWeakReference<TValue>> kvp in this.dictionary)
        {
            ExtendedWeakReference<TKey> weakKey = (ExtendedWeakReference<TKey>)(kvp.Key);
            ExtendedWeakReference<TValue> weakValue = kvp.Value;
    ...
    public void RemoveCollectedEntries()
    {
        ...
        foreach (KeyValuePair<object, ExtendedWeakReference<TValue>> pair in this.dictionary)
        {
            ExtendedWeakReference<TKey> weakKey = (ExtendedWeakReference<TKey>)(pair.Key);
            ExtendedWeakReference<TValue> weakValue = pair.Value;
    ...
    public class WeakKeyDictionary<TKey, TValue> : BaseDictionary<TKey, TValue>
        where TKey : class
    {
