namespace BaseTools.UI.Common
{
    ...
    /// <summary>
    /// Contains utils for working with visual tree.
    /// </summary>
    public static class VisualTreeHelperExtensions
    {
        ...
        [System.Diagnostics.CodeAnalysis.SuppressMessage("Microsoft.Design", "CA1011:ConsiderPassingBaseTypesAsParameters", Justification = "Third-party logic")]
        ...
        public static IEnumerable<FrameworkElement> GetVisualDescendants(this FrameworkElement root)
        {
        ...
        public static TChild GetVisualDescendant<TChild>(this FrameworkElement root) where TChild : FrameworkElement 
        {
            var childs = VisualTreeHelperExtensions.GetVisualDescendants(root);
            return (TChild)childs.FirstOrDefault(c => c is TChild);
        }
        ...
        [System.Diagnostics.CodeAnalysis.SuppressMessage("Microsoft.Globalization", "CA1305:SpecifyIFormatProvider", MessageId = "System.String.Format(System.String,System.Object)", Justification = "There are no culture dependent values.")]
        ...
        [System.Diagnostics.CodeAnalysis.SuppressMessage("Microsoft.Design", "CA1062:Validate arguments of public methods", MessageId = "0", Justification = "validated by guard.")]
        public static TRequestedTransform GetTransform<TRequestedTransform>(this UIElement element, TransformCreationModes mode) where TRequestedTransform : Transform, new()
        {
            ...
            {
                if ((mode & TransformCreationModes.Create) == TransformCreationModes.Create)
                {
            ...
            if (matrixTransform != null)
            {
                if (matrixTransform.Matrix.IsIdentity
                  && (mode & TransformCreationModes.Create) == TransformCreationModes.Create
                  && (mode & TransformCreationModes.IgnoreIdentityMatrix) == TransformCreationModes.IgnoreIdentityMatrix)
                {
                    requestedTransform = new TRequestedTransform();
                    element.RenderTransform = requestedTransform;
                    return requestedTransform;
            ...
            if (transformGroup != null)
            {
                foreach (Transform child in transformGroup.Children)
                {
                    ...
                    var requestedTransformChild = child as TRequestedTransform;
                    if (requestedTransformChild != null)
                    {
                        return requestedTransformChild;
                    }
                }

                // Right type was not found, but we are OK to add it
                if ((mode & TransformCreationModes.AddToGroup) == TransformCreationModes.AddToGroup)
                {
                    requestedTransform = new TRequestedTransform();
            ...
            if ((mode & TransformCreationModes.CombineIntoGroup) == TransformCreationModes.CombineIntoGroup)
            {
        ...
        [System.Diagnostics.CodeAnalysis.SuppressMessage("Microsoft.Design", "CA1062:Validate arguments of public methods", MessageId = "0", Justification = "Validated by guard")]
        [System.Diagnostics.CodeAnalysis.SuppressMessage("Microsoft.Design", "CA1004:GenericMethodsShouldProvideTypeParameter", Justification = "Needed for setting build-time restriction to type parameter.")]
        [System.Diagnostics.CodeAnalysis.SuppressMessage("Microsoft.Design", "CA1011:ConsiderPassingBaseTypesAsParameters", Justification = "Needed for setting build-time restriction to type parameter.")]
        public static string GetTransformPropertyPath<TRequestedType>(this FrameworkElement element, string subProperty) where TRequestedType : Transform
        {
            ...
            if (t is TRequestedType)
                return String.Format(CultureInfo.InvariantCulture, "(RenderTransform).({0}.{1})", typeof(TRequestedType).Name, subProperty);
            ...
            if (g != null)
            {
                for (int i = 0; i < g.Children.Count; i++)
                {
                    if (g.Children[i] is TRequestedType)
                        return String.Format(CultureInfo.InvariantCulture, "(RenderTransform).(TransformGroup.Children)[" + i + "].({0}.{1})",
        ...
        public static PlaneProjection GetPlaneProjection(this UIElement element, bool create)
        {
            ...
            var planeProjection = originalProjection as PlaneProjection;
    ...
    /// </summary>
    [Flags]
    public enum TransformCreationModes
    {
        /// <summary>
